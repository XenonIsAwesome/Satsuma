...
Per the SemVer spec's own [§4](https://semver.org/#spec-item-4), major version zero (`0.y.z`) is for initial development, where the public surface isn't yet considered stable: Satsuma was pre-1.0 through the phased build-out described in [docs/phase0.md](docs/phase0.md) and [docs/phases/](docs/phases/), so a breaking change during that period bumped MINOR rather than MAJOR. `1.0.0` was cut once real conversion (Phase 2) landed and the file-manager trigger contract was considered stable — from that point on, MAJOR is used as described above.
